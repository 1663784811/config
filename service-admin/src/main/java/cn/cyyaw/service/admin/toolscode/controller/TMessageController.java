...
import cn.cyyaw.service.admin.toolscode.table.entityconst.TMessageConst;
...
public class TMessageController {
     ...
     * 表:t_message ===> 所有:带条件
     */
    @RequestMapping(value = "/findAllTMessage", method = RequestMethod.GET)
    public void findAllTMessage(HttpServletResponse response, String jsonStr, SelectEntity selectEntity) {
        List<TMessage> list = tMessageService.findAll(jsonStr, selectEntity);
        ResponseUtils.responseJsonFilter(response, list, TMessageConst.filterselectColumnArr);
    }

    /**
     * 分页条件查询
     */
    @RequestMapping(value = "/findPageTMessage", method = RequestMethod.GET)
    public void findPageTMessage(HttpServletResponse response, String jsonStr, SelectEntity selectEntity) {
        PageRequest pageRequest = JpaUtils.getPageRequest(selectEntity);
        Page<TMessage> page = tMessageService.findPage(jsonStr, pageRequest);
        ResponseUtils.responseJsonFilter(response, PageUtil.pageFormat(page), TMessageConst.filterselectColumnArr);
    }

    /**
     * 根据ID查询
     */
    @RequestMapping(value = "/findIdTMessage", method = RequestMethod.GET)
    public void findIdTMessage(HttpServletResponse response, @RequestParam Integer id) {
        TMessage obj = tMessageService.findId(id);
        ResponseUtils.responseJsonFilter(response, obj, TMessageConst.filterselectColumnArr);
    }
     ...
     * 添加或修改
     */
    @PostMapping(value = "/saveTMessage")
    public void saveTMessage(HttpServletResponse response, @RequestBody TMessage tMessage) {
        TMessage obj = null;
        //添加
        Integer id = tMessage.getId();
        if (null == id) {
            //添加
            log.info("添加:{}", tMessage);
            WhyBeanUtils.filterField(tMessage, TMessageConst.filteraddColumnArr);
            tMessage.setCreatetime(new Date());
            tMessage.setTid(WhyStringUtil.getUUID());
            obj = tMessageService.save(tMessage);
        } else {
            //修改
            log.info("修改:{}", tMessage);
            TMessage tMessage1 = tMessageService.findId(id);
            Assert.notNull(tMessage1, "操作失败！");
            WhyBeanUtils.filterField(tMessage, TMessageConst.filteraddColumnArr);
            obj = tMessageService.save(tMessage);
        }
        ResponseUtils.responseJsonFilter(response, obj, TMessageConst.filterselectColumnArr);
    ...
    public Map delTMessage(@RequestBody Integer idArr[]) {
        tMessageService.del(idArr);
