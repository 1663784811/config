...
import cn.cyyaw.service.admin.toolscode.table.entityconst.TRoleConst;
...
public class TRoleController {
     ...
     * 表:t_role ===> 所有:带条件
     */
    @RequestMapping(value = "/findAllTRole", method = RequestMethod.GET)
    public void findAllTRole(HttpServletResponse response, String jsonStr, SelectModel selectModel) {
        List<TRole> list = tRoleService.findAll(jsonStr, selectModel);
        ResponseUtils.responseJsonFilter(response, list, TRoleConst.filterselectColumnArr);
    }

    /**
     * 分页条件查询
     */
    @RequestMapping(value = "/findPageTRole", method = RequestMethod.GET)
    public void findPageTRole(HttpServletResponse response, String jsonStr, SelectModel selectModel) {
        PageRequest pageRequest = JpaUtils.getPageRequest(selectModel);
        Page<TRole> page = tRoleService.findPage(jsonStr, pageRequest);
        ResponseUtils.responseJsonFilter(response, PageUtil.pageFormat(page), TRoleConst.filterselectColumnArr);
    }

    /**
     * 根据ID查询
     */
    @RequestMapping(value = "/findIdTRole", method = RequestMethod.GET)
    public void findIdTRole(HttpServletResponse response, @RequestParam Integer id) {
        TRole obj = tRoleService.findId(id);
        ResponseUtils.responseJsonFilter(response, obj, TRoleConst.filterselectColumnArr);
    }
     ...
     * 添加或修改
     */
    @PostMapping(value = "/saveTRole")
    public void saveTRole(HttpServletResponse response, @RequestBody TRole tRole) {
        TRole obj = null;
        //添加id
        Integer id = tRole.getId();
        if (null == id) {
            //添加
            WhyBeanUtils.filterField(tRole, TRoleConst.filteraddColumnArr);
            tRole.setCreatetime(new Date());
            tRole.setTid(WhyStringUtil.getUUID());
            obj = tRoleService.save(tRole);
        } else {
            //修改
            TRole tRole1 = tRoleService.findId(id);
            Assert.notNull(tRole1, "操作失败！");
            WhyBeanUtils.filterField(tRole, TRoleConst.filteraddColumnArr);
            obj = tRoleService.save(tRole);
        }
        ResponseUtils.responseJsonFilter(response, obj, TRoleConst.filterselectColumnArr);
     ...
     * 删除
     */
    @RequestMapping(value = "/delTRole")
    public Map delTRole(@RequestBody Integer idArr[]) {
